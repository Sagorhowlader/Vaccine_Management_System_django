...
from profiles.models import Profile, Feddback
...
def admin_dashboard_view(request):
    ...
    if val:
        ...
        dict = {
            ...
            'total_feedback': Feddback.objects.filter().count()
        }
...
def feedback_admin(request):
    form = Feddback.objects.all()
    # if request.POST:
    #     feedback = Feddback.objects.get(id = request.id)
    #     mail = feedback.Email
    return render(request, 'patient/feedback_admin.html', {'form': form})
